...
class AccountDetailsFragment : Fragment, OnDatabaseAccessListener<List<Account>> {
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        if(!accounts.isEmpty()) {
            txt_brita_account_balance.text = accounts[0].getAccountBalance().toString()
            txt_brita_coin_initials.text = accounts[0].getCoin().getCoinInitials()
            txt_bitcoin_account_balance.text = accounts[1].getAccountBalance().toString()
            txt_bitcoin_coint_initials.text = accounts[1].getCoin().getCoinInitials()
            txt_brl_account_balance.text = accounts[2].getAccountBalance().toString()
